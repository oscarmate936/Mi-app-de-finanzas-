...
# --- ESTILOS CSS (REDISEÑO FINTECH PROFESIONAL) ---
st.markdown("""
<style>
    ...
    div.stButton > button[kind="primary"] { /* GASTOS */
    ...
    /* Botón de Ingresos (Verde) */
    .stButton > button[key="btn_ing"] {
...
# Saldo Final Acumulado (Sueldo + Ingresos - Gastos)
saldo_final = sueldo_base + df[df['type']=='Ingreso']['amount'].sum() - df[df['type']=='Gasto']['amount'].sum()

# --- DIÁLOGOS ---
@st.dialog("🎯 Configurar Mi Pago")
def sueldo_dialog():
    s = st.number_input("Monto de mi pago", value=sueldo_base)
    f = st.date_input("Fecha en que recibí el pago")
    if st.button("Guardar mi Salario", use_container_width=True):
...
def ingreso_dialog():
    ...
    if st.button("Confirmar Ingreso", use_container_width=True):
        db["transactions"].append({
            ...
            "type": "Ingreso", "category": "Ingreso Extra", "amount": amt, "note": "Entrada de dinero"
        })
        save_db(db); st.rerun()

@st.dialog("🔴 Registrar Gasto")
def gasto_dialog():
    cat = st.selectbox("Categoría", ["Comida", "Entertainment", "Transporte", "Servicios", "Salud", "Ropa", "Otros"])
    amt = st.number_input("Monto ($)", min_value=0.0)
    note = st.text_input("¿En qué gastaste?")
...
# --- INTERFAZ (SIGUIENDO TU IMAGEN) ---

# 1. Tarjeta de Pago (Botón Superior)
pago_txt = db["settings"].get("pago", "---")
if st.button(f"💳 {pago_txt} \n\n SALARIO FIJO: ${sueldo_base:,.2f}", key="btn_salary", use_container_width=True, type="secondary"):
    sueldo_dialog()

# 2 y 3. Mosaicos de Mes (Ingresos y Gastos)
st.write("")
...
st.markdown(f"""<div class="delta-card">
    <div style="font-size:14px; opacity:0.8">Saldo Actual (Resumen Mes)</div>
...
if not df.empty:
    for _, row in df.sort_values('date', ascending=False).iterrows():
        ...
        if st.button(f"🗑️ Eliminar registro", key=f"del_{row['id']}", use_container_width=True):
            db["transactions"] = [t for t in db["transactions"] if t["id"] != row['id']]
            save_db(db); st.rerun()

# 6 y 7. Botones de Acción Inferiores
st.divider()
# Resultado final según tu indicación (Sueldo + todas las transacciones)
st.success(f"**TOTAL DISPONIBLE: ${saldo_final:,.2f}**")
